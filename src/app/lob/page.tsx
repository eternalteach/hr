  ...
  return (
    <div className="p-6 flex flex-col gap-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-gray-900">{t("nav.lob")}</h1>
          <p className="text-sm text-gray-500 mt-0.5">{t("lob.subtitle", { n: rows.length })}</p>
        </div>
        <button
        ...
        >
          <Plus className="w-4 h-4" />{t("lob.add")}
        </button>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => setDeleteTarget(null)}>
          <div className="bg-white rounded-xl shadow-xl w-full max-w-sm mx-4" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between p-5 border-b border-gray-100">
              <h2 className="text-base font-semibold text-gray-900">{t("lob.delete_title")}</h2>
            ...
            <div className="p-5 space-y-4">
              <div className="flex items-start gap-3 bg-amber-50 border border-amber-200 rounded-lg p-3 text-amber-700 text-sm">
                ...
                <p><span className="font-semibold">{deleteTarget.code}</span>{t("common.delete_suffix")}</p>
              </div>
