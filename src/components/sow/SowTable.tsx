      ...
      <table className="w-full text-sm whitespace-nowrap">
        <thead>
          <tr className="bg-gray-50 border-b border-gray-200 text-xs font-medium text-gray-500">
            {["SOW ID", "LOB", "타이틀(한글)", "타이틀(영문)", "내용(한글)", "내용(영어)", "비고(한글)", "비고(영어)", "마일스톤 시기", "유효여부", "최종수정일", ""].map(h => (
        ...
        <tbody>
          {rows.map(row => (
            <tr key={row.id} className="border-b border-gray-100 last:border-0 hover:bg-gray-50 transition-colors">
              ...
              <td className="px-4 py-3 max-w-[150px] truncate text-gray-700" title={row.title_ko ?? ""}>{cell(row.title_ko)}</td>
              <td className="px-4 py-3 max-w-[150px] truncate text-gray-700" title={row.title_en ?? ""}>{cell(row.title_en)}</td>
              <td className="px-4 py-3 max-w-[180px] truncate text-gray-700" title={row.content_ko}>{row.content_ko}</td>
              <td className="px-4 py-3 max-w-[180px] truncate text-gray-700" title={row.content_en}>{row.content_en}</td>
              <td className="px-4 py-3 max-w-[120px] truncate text-gray-500" title={row.note_ko ?? ""}>{cell(row.note_ko)}</td>
